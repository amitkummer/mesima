...
class TaskDetailTests(APITestCase):
    ...
    def test_patch(self):
        ...
        response = self.client.patch(
            reverse("task-detail", args=[self.task.id]), data=data
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_put_status_nested(self):
        response = self.client.put(
            reverse("task-status", args=[self.another_task.id]),
            data={"status": "active"},
        )
    ...
    def test_put_status_nested_400(self):
        response = self.client.put(
            reverse("task-status", args=[self.another_task.id]),
            data={"status": "not-a-valid-choice"},
        )
        self.assertEqual(response.status_code, 400)

    def test_put_status_nested_404(self):
        response = self.client.put(
            reverse("task-status", args=[1337]), data={"status": "active"}
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_put_owner_nested(self):
        response = self.client.put(
            reverse("task-owner", args=[self.another_task.id]),
            data={"ownerId": self.another_person.id},
        )
        self.assertEqual(response.status_code, 204)

    def test_put_owner_nested_400(self):
        response = self.client.put(
            reverse("task-owner", args=[self.another_task.id]), data={"ownerId": 1337}
        )
        self.assertEqual(response.status_code, 400)

    def test_put_owner_nested_404(self):
        response = self.client.put(
            reverse("task-owner", args=[1337]), data={"ownerId": self.person.id}
        )
        self.assertEqual(response.status_code, 404)
