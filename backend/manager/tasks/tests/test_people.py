...
class PersonDetailTests(APITestCase):
    ...
    def test_patch(self):
        response = self.client.patch(reverse('person-detail', args=[self.person.id]),
    ...
    def test_post_tasks_nested_404(self):
        response = self.client.post(reverse('person-tasks', args=[1337]),
            data={'title':'a tour of c++', 'details':'details', 'dueDate':date.today().isoformat(), 'status':'active'})
        self.assertEqual(response.status_code, 404)
