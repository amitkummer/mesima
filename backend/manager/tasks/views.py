...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def status(self, request, pk):
        try:
            ...
            return Response(
                f"A task with the id {pk} does not exist",
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                headers={"Location": f"tasks/{task.id}", "x-Created-Id": task.id},
...
class PersonViewSet(viewsets.ModelViewSet):
    ...
    def tasks(self, request, pk):
        try:
            ...
            return Response(
                f"A person with the id {pk} does not exist",
                status=status.HTTP_404_NOT_FOUND,
            )
        if request.method == "GET":
            ...
            if person_tasks.count() < 1:
                return Response(
                    data="Tasks do not exist", status=status.HTTP_404_NOT_FOUND
                )
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                headers={"Location": f"people/{person.id}", "x-Created-Id": person.id},
                status=status.HTTP_201_CREATED,
            )
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
